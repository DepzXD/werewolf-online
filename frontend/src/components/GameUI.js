...
export default function GameUI({ state, socketRef, playerName, room }) {
  ...
  const showConfirm =
    state.game.confirms[playerName] !== undefined &&
    !state.game.confirms[playerName];
  return (
    <React.Fragment>
      <ModelStyles>
        <div>
        ...
        <div className="deck">
          {state.game.roles.center.map((_card, i) => (
            <Card
              ...
              selected={
                state.game.actions &&
                state.game.actions.filter(
                  (action) => action[0] === 'center' && action[1] === i
                ).length > 0
              }
              key={i}
            />
              ...
              <div className="flx" key={i}>
                ...
                <Card
                  ...
                  selected={
                    ...
                    state.game.actions.filter(
                      (action) =>
                        action[0] === 'playerRoles' && action[1] === player
                    ).length > 0
                  }
                />
              </div>
