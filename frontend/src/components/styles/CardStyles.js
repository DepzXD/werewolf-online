...
const CardStyle = styled.div`
  ...
  ${(props) =>
    ...
    css`
      height: 27rem;
      width: 17rem;
      border: 4px solid ${({ theme }) => theme.myCard};
    `}
  ${(props) =>
    props.deck &&
    css`
      height: 9rem;
      width: 6rem;
      border: 4px solid ${({ theme }) => theme.centerCard};
    `}
  ${(props) =>
    props.player &&
    css`
      border: 4px solid ${({ theme }) => theme.playerCard};
      height: 12rem;
      width: 8rem;
    `}
  ${({ selected }) =>
    selected &&
    css`
      border: 5px solid ${({ theme }) => theme.selectedCard};
    `} 
